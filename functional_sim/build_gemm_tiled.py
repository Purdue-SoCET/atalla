...
def make_tiled_gemm_asm(M: int, N: int, K: int) -> str:
    ...
    # Pack once (SID1=C, SID0=A/B); tmp_a/tmp_b are scratch only during each pack.
    blk_c = sdma_block(57, 1, mr_m, mr_n, N)
    blk_a = sdma_block(48, 0, mr_m, mr_k, K)
    blk_b = sdma_block(49, 0, mr_k, mr_n, N)
...
def main():
    ...
    expected = systolic_gemm_vv_dram_reference(A, B)
    print(f"Tiled GEMM: C[{M},{N}] = A[{M},{K}] * B[{K},{N}] (gemm.vv + lw.vi reference)")
