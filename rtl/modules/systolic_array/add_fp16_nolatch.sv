...
always_comb begin : special_case_detect
    ...
    if (is_nan_a || is_nan_b) begin
        // NaN propagation - use canonical qNaN (same as multiplier)
        special_case = 1'b1;
...
always_comb begin : align_operands
    ...
    if (exp_diff >= 5'd13) begin
        ...
        // sticky_lost: bits shifted out, but bit0 was already 1 thus we subtract less than true thus result too large
...
always_comb begin : exp_adjust
    if (normalized_mantissa_sum == 13'd0) 
        ...
        exp_norm_s = $signed({1'b0, exp_max}) - $signed({3'b0, norm_shift});
end
