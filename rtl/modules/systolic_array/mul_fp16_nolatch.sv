...
module mul_fp16_nolatch(input logic [15:0] a, b, output logic [15:0] result);
    ...
    assign exp_overflow = exp_ovf | (mul_significand_rounded[10] & (exp_sum == 5'd30));
    // Underflow: adder signals when result exp <= 0 (subnormal -> flush to zero)
    // Exception: if rounding overflow pushes exp from 0 to 1, it's valid
    assign boundary_case = exp_unf & mul_significand_rounded[10];
    assign exp_underflow = exp_unf & ~mul_significand_rounded[10];
