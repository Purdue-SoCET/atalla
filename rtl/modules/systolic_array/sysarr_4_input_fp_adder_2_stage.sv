...
module sysarr_4_input_fp_adder_2_stage #(
    ...
    // =================================================================================
    // STAGE 2: Final Add, Normalization, and Rounding (Critical Path Isolation)
    // =================================================================================
    ...
    always_comb begin : stage2_logic
        ...
        // Exponent Adjustment
        final_exp_calc = $signed({2'b00, st1_a_e}) + 2 - $signed({2'b00, lead_zeros}) + BIAS_DIFF;

        // 4. Output Packing
        if (mag_sum == 0 || final_exp_calc <= 0 || st1_a_e == 0) result_out = {1'b0, {RES_WIDTH-1{1'b0}}};
        ...
        if (st1_special_case) result_out = st1_special_result;
    end
