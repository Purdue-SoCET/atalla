module add_4in_stage1 #(
    ...
    output logic [EXP-1:0] x_e_f,
    output logic x_s_f,
    
    output logic [(1 + MANTISSA + PRECISION + 1)-1:0] x_f,
    output logic [(1 + MANTISSA + PRECISION + 1)-1:0] y_shifted_f,
    output logic [(1 + MANTISSA + PRECISION + 1)-1:0] m_shifted_f,
    output logic [(1 + MANTISSA + PRECISION + 1)-1:0] n_shifted_f,
    
    output logic y_op_f, m_op_f, n_op_f,
    
    output logic special_case_f,
    output logic [15:0] special_result_f
);
    ...
    always_comb begin
        ...
        is_nan_a = (exp_a == {EXP{1'b1}}) && (frac_a != 0);
        is_nan_b = (exp_b == {EXP{1'b1}}) && (frac_b != 0);
        is_nan_c = (exp_c == {EXP{1'b1}}) && (frac_c != 0);
        is_nan_d = (exp_d == {EXP{1'b1}}) && (frac_d != 0);

        is_inf_a = (exp_a == {EXP{1'b1}}) && (frac_a == 0);
        is_inf_b = (exp_b == {EXP{1'b1}}) && (frac_b == 0);
        is_inf_c = (exp_c == {EXP{1'b1}}) && (frac_c == 0);
        is_inf_d = (exp_d == {EXP{1'b1}}) && (frac_d == 0);
        ...
        else if (is_inf_a || is_inf_b || is_inf_c || is_inf_d) begin
            logic has_pos_inf, has_neg_inf;
            has_pos_inf = (is_inf_a && !sign_a) || (is_inf_b && !sign_b) || 
                          (is_inf_c && !sign_c) || (is_inf_d && !sign_d);
            has_neg_inf = (is_inf_a && sign_a) || (is_inf_b && sign_b) || 
                          (is_inf_c && sign_c) || (is_inf_d && sign_d);
