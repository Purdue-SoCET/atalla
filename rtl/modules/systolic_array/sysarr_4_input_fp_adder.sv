...
module sysarr_4_input_fp_adder #(
    ...
    always_comb begin : stage2_logic
        sum_a = {2'b00, st1_x_f}; sum_b = {2'b00, st1_y_f}; sum_c = {2'b00, st1_m_f}; sum_d = {2'b00, st1_n_f};
        neg_b = st1_b_op ? ~sum_b : sum_b; neg_c = st1_c_op ? ~sum_c : sum_c; neg_d = st1_d_op ? ~sum_d : sum_d;
        ...
        if (raw_sum[SUM_WIDTH-1]) begin mag_sum = ~raw_sum + 1'b1; res_sign = ~st1_a_s; end
        else begin mag_sum = raw_sum; res_sign = st1_a_s; end
        lead_zeros = SUM_WIDTH - 1; // Correct default
        for (int i = SUM_WIDTH-1; i >= 0; i--) if (mag_sum[i]) begin lead_zeros = SUM_WIDTH - 1 - i; break; end
    end
    ...
    // --- Stage 3: The Precision Fix ---
    always_comb begin : stage3_logic
        norm_val = st2_sum_mag << st2_lzd;
        
        // Use hard offsets relative to the normalization position (SUM_WIDTH-1)
        ...
        // Ensure all lower bits + original alignment sticky are captured
        ...
        // The exp_base is the exponent of x_mant, which was at SUM_WIDTH-3 (because of 2-bit growth)
