...
module sysarr_4inp_fp_adder_2nd_pipeline_state #(
    ...
    input logic [MANTISSA_SIZE + PRECISION_BITS:0] a_f, b_f, c_f, d_f, // these include the precision bits
    ...
    output logic [1:0] right_shift_radix, 
);  
    ...
    always_ff @( posedge clk, negedge nRST ) begin 
        if(~nRST) begin
            ...
            sum_i <= {corrected_sum[WIDTH:1], corrected_sum[0] | b_sb | c_sb | d_sb};
    ...
    always_comb begin : right_shift_logic
        // Correct sum is the magnitude
        if (corrected_sum[WIDTH]) begin
            right_shift_radix = 2'd2; 
        end
        else if (corrected_sum[WIDTH-1]) begin
            right_shift_radix = 2'd1; 
        end
        else begin
            right_shift_radix = 2'd0; 
        end
    end
