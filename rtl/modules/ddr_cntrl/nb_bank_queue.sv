...
module nb_bank_queue(
    ...
    bq_slot_t bq_slot;
    assign bq_slot = {bqif.fe_r, bqif.fe_c, bqif.fe_write, bqif.fe_id};

    sync_fifo #(.DEPTH(BANK_NUM), .DWIDTH($bits(bq_slot_t))) bq_fifo (
        .clk(CLK), .rstn(nRST),
        .wr_en(bqif.fe_write_bq),
        .din(),
        .rd_en(~bqif.fe_write_bq),
