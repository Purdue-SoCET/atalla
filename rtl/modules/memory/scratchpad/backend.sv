...
module backend #(parameter logic [scpad_pkg::SCPAD_ID_WIDTH-1:0] IDX = '0) (
    ...
    always_comb begin
        ...
        if(busy || (bshif.sched_req[IDX].valid && !busy)) begin
            ...
            be_internal.be_dr_req_q_in.dram_addr = bshif.sched_req[IDX].dram_addr + row_base_addr + {sub_uuid, 2'b00};
            ...
            if(be_internal.be_dr_req_q_out.burst_complete == 1'b1) begin
                ...
                if(sub_uuid == num_request) begin
                    ...
                    if(bshif.sched_req[IDX].write == 1'b0) begin 
                        nxt_uuid = uuid + 1;
                        nxt_row_base_addr = row_base_addr + bshif.sched_req[IDX].full_num_cols + 1;
            ...
            if(bshif.sched_req[IDX].write == 1'b1) begin
                ...
                bdrif.be_dram_req[IDX].dram_addr = bshif.sched_req[IDX].dram_addr + row_base_addr + {sub_uuid, 2'b00};
            ...
            if(bshif.sched_req[IDX].write == 1'b1) begin
                if((schedule_request_counter == bshif.sched_req[IDX].num_rows) && (be_internal.be_dr_req_q_out.transaction_complete == 1'b1)) begin
                    nxt_busy = 1'b0;
                    nxt_uuid = 0;
                    nxt_sub_uuid = 0;
                ...
                if((schedule_request_counter == bshif.sched_req[IDX].num_rows) && (be_internal.sr_wr_l_out.sram_write_req_latched == 1'b1)) begin
                    nxt_busy = 1'b0;
                    nxt_uuid = 0;
                    nxt_sub_uuid = 0;
                    nxt_schedule_request_counter = 0;
