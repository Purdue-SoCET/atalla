...
module simple_systolic_model #(
    ...
    genvar i;
    generate 
        for (i = 0; i < LATENCY; i++) begin : VALID_PIPE
            always_ff @(posedge CLK or negedge nRST) begin
                if (!nRST) begin
                    valid_shift[i] <= 1'b0;
                end else begin
                    if (i == 0)
                        valid_shift[i] <= output_ready;
                    else
    ...
    assign output_valid = valid_shift[LATENCY-1];

    always_ff @(posedge CLK or negedge nRST) begin
        if (!nRST) begin
            data <= '0;
        end else begin
            // simulate 64-cycle systolic array “compute”
            if (output_valid)
                data <= $random;
        end
    end

    assign output_data = output_valid ? data : '0;
