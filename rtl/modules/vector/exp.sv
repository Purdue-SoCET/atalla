...
`include "ex_if.vh"
// THIS IS NOT THE REAL EXPONENTIAL UNIT. THIS IS A SQRT UNIT THAT SERVES AS A DUMMY FILE TO TEST 
// THE EXPONTENTIAL TESTBENCH
module exp (
    input  logic        CLK,
    input  logic        nRST,
    exp_if.exif        exif
    ...
    //input determination
    always_comb begin
        if (exif.valid_data_in & exif.ready) begin
            sign_n = exif.input_val.sign;
            exp_n = exif.input_val.exp;
            frac_n = exif.input_val.frac;
            slope_n = normal_slopes[exif.input_val.frac[9:6]];
            intercept_n = normal_intercepts[exif.input_val.frac[9:6]];
            valid_n = 'b1;
        end
        else begin
            sign_n = sign;
    ...
    always_ff @(posedge CLK or negedge nRST) begin
        if (!nRST)
            ready_reg <= 1'b1;
        else if (exif.valid_data_in & ready_reg)
            ready_reg <= 1'b0;
        else if (exif.valid_data_out)
            ready_reg <= 1'b1;
    end

    // For testbench/interface connection
    assign exif.ready = ready_reg;
    ...
    always_ff @(posedge CLK or negedge nRST) begin
        ...
        else if (exif.valid_data_out) begin
            // Clear after output
            special_flag_latched   <= 1'b0;
            special_result_latched <= 16'h0000;
        end
    ...
    // Output muxing
    assign exif.valid_data_out = valid_out_delay;
    assign exif.output_val     = special_flag_latched
