...
(
    ...
    always_comb begin
        ...
        if(halt_latch && !ex_if.scalar_halt_ready && !ex_if.vector_halt_ready && !ex_if.mask_halt_ready && unit5_if.ready_in && !ex_if.scpads_busy) begin
            ex_if.halt_out = halt_latch;
        end else begin
            ex_if.halt_out = 1'b0;
        end
        ex_if.internal_halt = n_halt_latch | halt_latch;
    end
