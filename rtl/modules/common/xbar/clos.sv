...
module clos #(
    ...
    localparam int IM_OM_SIZE = CLOS_SIZE / IM_OM_NUM,
    localparam int IM_OM_NUM_TAG  = $clog2(IM_OM_NUM),
    localparam int IM_OM_SIZE_TAG = $clog2(IM_OM_SIZE),

    localparam int CM_SIZE = CLOS_SIZE / CM_NUM,
    localparam int CM_SIZE_TAG = $clog2(CM_SIZE)
) (
    xbar_if.xbar xif
);
    ...
    logic [TAGWIDTH-1:0] n_center_perm   [CM_NUM-1:0][CM_SIZE-1:0];
    ...
    generate
        for (i = 0; i < CM_NUM; i++) begin
            for (j = 0; j < CM_SIZE; j++) begin
                assign output_module[j][i] = center_module[i][j];
                assign output_perm[j][i] = center_perm[i][j];
            end
        end
    endgenerate
    ...
    always_comb begin : n_center_comb
        for (int ii = 0; ii < IM_OM_NUM; ii++) begin
            num_counter[ii] = '0;
        end

        // init center next-state arrays
        for (int ii = 0; ii < CM_NUM; ii++) begin
            for (int jj = 0; jj < CM_SIZE; jj++) begin
                n_center_module[ii][jj] = '0;
                n_center_perm[ii][jj] = '0;
            end
        end

        for (int ii = 0; ii < IM_OM_NUM; ii++) begin
            for (int jj = 0; jj < IM_OM_SIZE; jj++) begin
                om_dest = (input_perm[ii][jj] >> 2);

                n_center_module[num_counter[om_dest]][om_dest] = input_module[ii][jj];
                n_center_perm[num_counter[om_dest]][om_dest] = input_perm[ii][jj];
