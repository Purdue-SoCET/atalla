...
module addsub_bf16(input logic clk, nRST, op,
...
always_comb begin
    if(bf1_in[14:7] < bf2_modified[14:7]) begin     // bf2 has a bigger exponent.
        smaller_exponent = bf1_in[14:7];
        larger_exponent = bf2_modified[14:7];
        exp_select = 1'b0;
    end
    else begin                                  // bf1 has a bigger exponent.
        smaller_exponent = bf2_modified[14:7];
...
always_comb begin
    ...
    if(exp_select == 0) begin                       // bf2 had a bigger exponent: shift bf1.
        ...
        frac_not_shifted = ~|bf2_modified[14:7] ? 10'b0 : {frac_leading_bit_bf2, bf2_modified[6:0], 2'b00};
        sign_not_shifted = bf2_modified[15];
        exp_max = bf2_modified[14:7];
    end
        ...
        if(exp_diff[7] | ~|bf1_in[14:7]) begin
            ...
            frac_shifted = {frac_leading_bit_bf2, bf2_modified[6:0], 2'b00} >> exp_diff[3:0];
        end
        // rounding_loss = |({frac_leading_bit_bf2, floating_point2_in[9:0], 2'b00} & ((1 << unsigned_exp_diff) - 1));
        sign_shifted = bf2_modified[15];
...
always_comb begin
    ...
    if (~|bf1_in[14:0] & ~|bf2_modified[1:0]) begin 
        result_sign = bf1_in[15] & bf2_modified[15];
    end
...
always_comb begin
    if (exp_select == 0) begin
        ...
        sticky_bit = |(({frac_leading_bit_bf2, bf2_modified[6:0], 2'b00} & ((1 << exp_diff) - 1)));
    end
end
...
logic [7:0] exp_out_adj;
    ...
    logic [15:7] xor_out; 
    assign xor_out = bf1_in[15:7] ^ bf2_modified[15:7]; 
    // Final output packing: sign(1) + exponent(8) + fraction(7) = 16 bits
    always_comb begin 
        // +inf & -inf 
        if (is_inf1 & is_inf2 & (bf1_in[15] ^ bf2_modified[15])) begin 
            bf_out = 16'h7fc0; 
        // overflow and underflow 
        end else if ((&bf1_in[14:8] & ~bf1_in[7] & &bf1_in[6:0] & |bf2_modified[14:0]) | (&bf2_modified[14:8] & ~bf2_modified[7] & &bf2_modified[6:0] & |bf1_in[14:0]) & ~(bf1_in[15] ^ bf2_modified[15])) begin 
