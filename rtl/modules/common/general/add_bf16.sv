...
lzd_10b normalizer(.fraction(mantissa_sum[9:0]), .result(normalized_mantissa_sum), .shifted_amount(norm_shift));
...
logic [7:0] exp_out_adj;
    ...
    always_comb begin
        if(overflow || invalid) begin
            final_exp = 8'b11111111;
        end
        else begin
            final_exp = exp_out_adj;
        end

        if(invalid) begin
            final_fraction = 7'b1000000;
        end
        else if(overflow) begin
            final_fraction = 7'b0000000;
        end
        else begin
            final_fraction = rounded_fraction;
        end
    end

    assign bf_out = {result_sign, final_exp, final_fraction};
