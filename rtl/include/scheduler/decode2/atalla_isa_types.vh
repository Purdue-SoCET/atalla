...
package atalla_isa_pkg;
  ...
  //scalar
  parameter SCALAR_REG_W = 8; //256 registers
  ...
  parameter NUM_ROWS = 5; //for sdma
  parameter NUM_COLS = 5; //for sdma
  //vector
  ...
  //   INVALID       = 4'b1011   //11 - invalid instr
  // } scalar_fu_type_t;
  ...
  typedef enum logic [OP_W-1:0] {
    ...
    SCPAD_LD   = 7'b1011000, //Scratchpad Load 
    SCPAD_ST   = 7'b1011001 //Scratchpad Store
  } opcode_t;


typedef enum logic [SCALAR_FU_W-1:0] {
    alu_valid        = 4'b0000,
    control_valid    = 4'b0001,
    bf_add_valid     = 4'b0010,
    bf_sub_valid     = 4'b0011,
    bf_mult_valid    = 4'b0100,
    bf_slt_valid     = 4'b0101,
    bf_div_valid     = 4'b0110,
    s_div_valid      = 4'b0111,
    s_mod_valid      = 4'b1000,
    BF_to_int_valid  = 4'b1001,
    int_to_BF_valid  = 4'b1010,
    ld_valid         = 4'b1011,
    st_valid         = 4'b1100,
    sMult_valid      = 4'b1101
} scalar_fu_enable_t;
  ...
  typedef struct packed {
    ...
    scalar_fu_enable_t fu_enable; //individual FU control signals for execute xbar
    logic valid_in; //is instruction valid or nop
    logic imm_src; //0 if no immediate used, 1 if immediate used
    logic halfword; //bf16 mem operation
    word_t imm;
    word_t incr7; //for branch instrs
    opcode_t op;
    logic halt;

    logic [SCALAR_REG_W-1:0] rs1;            
    logic [SCALAR_REG_W-1:0] rs2;              
    logic [SCALAR_REG_W-1:0] rdIn; 

    word_t r1_data, r2_data; 

    logic use_rs1; // Does instruction read rs1?
    logic use_rs2; // Does instruction read rs2?
    logic reg_write;
  ...
  typedef struct packed {
    ...
    logic [VECTOR_REG_W-1:0] vs1;            
    logic [VECTOR_REG_W-1:0] vs2;              
  ...
  typedef struct packed {
    ...
    logic valid; //valid instr
    logic sid; //scrachpad id
    logic store; //1 if store
