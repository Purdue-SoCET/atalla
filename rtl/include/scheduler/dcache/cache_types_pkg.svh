...
`define CACHE_TYPES_PKG_VH
    ...
    localparam BANKS_LEN = (NUM_BANKS > 1) ? $clog2(NUM_BANKS) : 1; 
    ...
    typedef enum logic [4:0] { 
        START, ADDRESS_READ, ADDRESS_WAIT, ADDRESS_CALC, 
        LRU_CALC, BUFFER_WRITE, BUFFER_WAIT, BUFFER_STATE, 
        BLOCK_PULL, VICTIM_EJECT, FINISH_WRITE, FINISH_WAIT, FINISH, 
        FLUSH_READ, FLUSH_WAIT, FLUSH, WRITEBACK, WRITEBACK_WRITE, WRITEBACK_WAIT, HALT
    } bank_fsm_states; 

    typedef enum logic [2:0] {
        HC_IDLE, HC_WAIT_READ, HC_EVAL, HC_WAIT_WRITE
    } hc_fsm_states;
