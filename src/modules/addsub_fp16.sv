...
module addsub_fp16(input logic clk, nRST, start,
...
assign fp2_eff = {op ? ~fp2_in[15] : fp2_in[15], fp2_in[14:0]};

// i hope the case where the two exponents are equal isn't an issue?
always_comb begin
    if(fp1_in[14:10] < fp2_eff[14:10]) begin     // fp2 has a bigger exponent.
        smaller_exponent = fp1_in[14:10];
        larger_exponent = fp2_eff[14:10];
        exp_select = 1'b0;
    end
    else begin                                  // fp1 has a bigger exponent.
        smaller_exponent = fp2_eff[14:10];
...
always_comb begin
    ...
    if(exp_select == 0) begin                       // fp2 had a bigger exponent: shift fp1.
        ...
        frac_not_shifted = {frac_leading_bit_fp2, fp2_eff[9:0], 2'b00};
        sign_not_shifted = fp2_eff[15];
        exp_max = fp2_eff[14:10];
    end
        ...
        if(exp_diff[4]) begin
            ...
            frac_shifted = {frac_leading_bit_fp2, fp2_eff[9:0], 2'b00} >> exp_diff[3:0];
        end
        // rounding_loss = |({frac_leading_bit_fp2, floating_point2_in[9:0], 2'b00} & ((1 << unsigned_exp_diff) - 1));
        sign_shifted = fp2_eff[15];
...
left_shift_fp16 normalizer(.fraction(mantissa_sum[12:0]), .result(normalized_mantissa_sum), .shifted_amount(norm_shift));
