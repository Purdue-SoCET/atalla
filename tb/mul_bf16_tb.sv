...
// Command: verilator --binary -j 0 -Wall -Wno-fatal mul_bf16_tb.sv ./../rtl/mul_bf16.sv -Imodules -Itestbench -Iinclude --hierarchical --trace; ./obj_dir/Vmul_bf16_tb; gtkwave ../waves/mul_bf16_waves.vcd --save=./../waves/mul_bf16_debug.gtkw 
`timescale 1ns/1ps
module mul_bf16_tb;
    logic clk;
    logic nRST;
    ...
    initial begin
        ...
        $dumpvars(0, mul_bf16_tb);

        // open generated test file (3-column format: a b result)
        fd = $fopen("testcases/random_bf16_mul_cases.txt", "r");
        if (fd == 0) begin
        ...
        fd_out = $fopen("result_output/bf16_mul_test_results.txt", "w");
        if (fd_out == 0) begin
            $display("ERROR: cannot open bf16_mul_test_results.txt for writing");
            $fclose(fd);
            $finish;
        end
