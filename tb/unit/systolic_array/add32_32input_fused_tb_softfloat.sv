...
module add32_32input_fused_tb_softfloat;
    ...
    // --- Params ---
    localparam PERIOD = 2;
    localparam LATENCY = 1; // Adjust this based on the actual pipeline depth of the aligner/adder
    localparam GRS = 1;
    localparam PRECISION_BITS = 3;
    
    // FP32 Internal Format
    ...
    typedef struct {
        logic [15:0] inputs [31:0]; // Unpacked to match module
        logic [15:0] expected;
        int id;
    } scoreboard_entry_t;
    ...
    // DUT Signals
    logic [15:0] tb_inputs [31:0]; // 32 BF16 inputs
    logic [31:0] fp32_result;      // Intermediate FP32 result from the adder
    logic [15:0] tb_result;        // Final BF16 result after reduction

    // --- DUT 1: The 32 Input Adder ---
    sysarr_32_input_adder #(
    ...
    // --- DUT 2: The FP32 to BF16 Reducer ---
    reducer #(
        .IN_EXP_W(EXPONENT_SIZE),     // 8
        .IN_MANT_W(MANTISSA_SIZE),    // 23
        .OUT_EXP_W(IN_EXPONENT_SIZE), // 8  (BF16 Exponent)
        .OUT_MANT_W(IN_MANTISSA_SIZE) // 7  (BF16 Mantissa)
    ) dut_reduce (
        .fp_in(fp32_result),
        .fp_out(tb_result)
    );
    ...
    function automatic logic [7:0] get_exponent(input logic [15:0] val);
        return val[14:7];
    endfunction
    ...
    initial begin
        ...
        tb_nrst = 1'b0;
        for (int i=0; i<32; i++) tb_inputs[i] = '0;
        #(PERIOD * 5);
        tb_nrst = 1'b1;
