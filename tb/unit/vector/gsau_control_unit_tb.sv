...
module gsau_control_unit_tb;
  ...
  simple_systolic_model #(.LATENCY(96)) systolic_array (
    .CLK(CLK),
    .nRST(nRST),
    .output_ready(gsau_port.sa_input_en),
    .output_valid(gsau_port.sa_valid_in),
  ...
  initial begin
    gsau_port.sa_ready_in = 1;
    //gsau_port.sa_valid_in = 0;
    //gsau_port.sa_array_output = '0;
    gsau_port.wb_ready_in = 1;
  ...
  initial begin
    ...
    test = "0 Power On Reset";
    assert(!gsau_port.wb_valid_out) else $error("TEST 0 WB valid not cleared!");
    ...
    repeat (5) begin
      ...
      toggle_wb_ready(1);
      repeat (2) @(posedge CLK);
      assert (gsau_port.wb_valid_out == 0)
        else $error("TEST 7 wb_valid_out asserted during ready toggle");
    end    
    repeat (10) @(posedge CLK);
    ...
    repeat (10) @(posedge CLK);
    assert (gsau_port.wb_valid_out == 0)
      else $error("TEST 8 wb_valid_out asserted while FIFO empty");
    assert (gsau_port.sa_input_en == 0)
      else $error("TEST 8 sa_input_en asserted while FIFO empty");
    repeat (10) @(posedge CLK);

    // Test 9: FIFO Write Pointer Wrap-Around
    $display("[T9] FIFO Write Pointer Wrap-Around");
    test = "9 pointer wrap";

    // Force FIFO to have space always
    //gsau_port.sa_ready_in = 1;

    // Fill FIFO until pointer reaches LAST entry
    for (int i = 0; i < FIFOSIZE; i++) begin
        send_scoreboard_entry($urandom_range(0,255), 0, $urandom, $urandom);
        @(posedge CLK);

        // if (i == FIFOSIZE - 1) begin
        //   // At the final location, pointer MUST wrap to 0
        //   if (dut.<your_fifo_inst>.wptr !== 0) begin
        //     $error("[T9] FIFO write pointer FAILED to wrap! wptr=%0d", 
        //             dut.<your_fifo_inst>.wptr);
        //   end else begin
        //     $display("[T9] FIFO write pointer wrapped correctly to 0.");
        //   end
        // end
    end
