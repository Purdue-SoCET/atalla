...
module vreduction_tb;
    ...
    task set_DUT_inputs(
        ...
        // Drive inputs through the struct member .in
        vruif.in.vector_input   = input_vector;
        vruif.in.lane_input     = input_vector[WIDTH-1:0];
        vruif.in.reduction_type = op;
        vruif.in.broadcast      = broadcast;
        vruif.in.clear          = clear;
        vruif.in.valid_in       = valid_data_input;
        vruif.in.imm            = index;
        @(posedge CLK);
    endtask
...
function automatic fp16_t find_min(fp16_t [WIDTH-1:0] tree_inputs);
    ...
    task clear();
        // Use .in members for input-side defaults
        vruif.in.valid_in       = 0;
        vruif.in.broadcast      = 0;
        vruif.in.clear          = 0;
        vruif.in.imm            = 0;
        vruif.in.reduction_type = 2'b00;
    endtask
...
endtask
    ...
    initial begin
        ...
        // Make sure testbench indicates it's ready to accept DUT outputs
    ...
    always @(posedge CLK) begin
        ...
        // Read DUT outputs via vruif.out.<field>
        if (vruif.out.valid_out) begin
            expected_line = "";
            actual_line = "";
            diff_line = "";
            test_pass = 1;
            ...
            for (int i = 0; i < NUM_ELEMENTS; i++) begin
                ...
                automatic logic [15:0] act_bits = vruif.out.vector_output[i];
