module vlsu_tb;
    ...
    localparam int SP_LATENCY = 12; // Real scratchpad: FE(0) + head(0) + wxbar(~3) + cntrl(~2) + SRAM(2) + rxbar(~3) + tail(0)
    ...
    initial begin
        ...
        begin
            ...
            // Collect with toggling wb_ready (50% duty)
            for (int r = 0; r < 4; r++) begin
                int waited;
                waited = 0;
                while (waited < SP_LATENCY + FIFO_DEPTH * 4 + 30) begin
                    ...
                    @(posedge CLK);
                    waited++;
                    if (`WB_OUT.valid && `WB_READY) break;
                end
