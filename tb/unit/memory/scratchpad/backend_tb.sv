...
program test (
    ...
    task automatic dram_results_with_full_check();
        ...
        begin
            // Wait for one clock edge to let backend see the sched_req
            @(posedge bif.clk);
            ...
            // Wait for completion
            timeout_counter = 0;
            while (bif.sched_res[0].valid !== 1'b1) begin
                if (bif.be_req[0].valid && bif.be_req[0].write) begin
                    verify_be_req_write(sram_rows_received, num_cols, base_spad_addr);
                    sram_rows_received++;
                    clear_expected_row();
    ...
    task automatic sram_store_with_full_check();
        ...
        begin
            ...
            // Check for combinational be_req immediately after sched_req
            #0;
            if (bif.be_req[0].valid && !bif.be_req[0].write) begin
                automatic logic [SCPAD_ADDR_WIDTH-1:0] expected_spad_addr;
                store_be_req_count++;
