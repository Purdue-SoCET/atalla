...
* @brief Supports variable input size, variable input channels, variable kernel size, variable stride, variable padding and no dilation.
*/
...
*/
FILE* f_timeline;
long pe_heatmap[SA_TK][SA_TN]; 
long total_theoretical_ops = 0;
long total_actual_ops = 0;
float peak_util_pct = 0.0f; // New: Track Peak
int gemm_step_count = 0;

void init_analysis() {
    f_timeline = fopen("conv_prog_model/utilization_over_time.csv", "w");
    if (f_timeline) fprintf(f_timeline, "Step,Active_M,Active_N,Active_K,Utilization_Percent\n");
    memset(pe_heatmap, 0, sizeof(pe_heatmap));
    total_theoretical_ops = 0; 
    total_actual_ops = 0; 
    peak_util_pct = 0.0f; // Reset Peak
    gemm_step_count = 0;
}

void log_gemm_step(int active_M, int active_N, int active_K) {
    long capacity = SA_TM * SA_TN * SA_TK; 
    long actual   = active_M * active_N * active_K;
    float util_pct = (float)actual / (float)capacity * 100.0f;
    
    // Track Peak
    if (util_pct > peak_util_pct) {
        peak_util_pct = util_pct;
    }
    
    if (f_timeline) fprintf(f_timeline, "%d,%d,%d,%d,%.2f\n", gemm_step_count++, active_M, active_N, active_K, util_pct);
    
    for (int k = 0; k < SA_TK; k++) {
        for (int n = 0; n < SA_TN; n++) {
            if (k < active_K && n < active_N) {
                pe_heatmap[k][n] += active_M; 
            }
        }
    }
    
    total_theoretical_ops += capacity; 
    total_actual_ops += actual;
}

void print_utilization() {
    if (total_theoretical_ops == 0) return;
    float avg_util = (float)total_actual_ops / (float)total_theoretical_ops * 100.0f;
    printf("\n[ATALLA PERFORMANCE]\n");
    printf("  Total GEMM Steps: %d\n", gemm_step_count);
    printf("  Active MAC Ops  : %ld\n", total_actual_ops);
    printf("  Peak Utilization: %.2f%%\n", peak_util_pct); // Prints Peak
    printf("  Avg  Utilization: %.2f%%\n", avg_util);      // Prints Average
    printf("=====================\n\n");
}

void dump_heatmap_and_close() {
    if (f_timeline) fclose(f_timeline);
    FILE* f_map = fopen("conv_prog_model/pe_heatmap.csv", "w");
    if (f_map) {
        for (int r = 0; r < SA_TK; r++) { 
            for (int c = 0; c < SA_TN; c++) fprintf(f_map, "%ld%s", pe_heatmap[r][c], (c < SA_TN - 1) ? "," : "");
            fprintf(f_map, "\n");
        }
        fclose(f_map);
    }
...
static float* flatten_kernels_to_B_matrix(KernelTensor* kernels) {
    // Flatten [C_out, C_in, K_h, K_w] -> [K_flat, C_out]
    // K_flat includes ALL input channels: K_flat = C_in * K_h * K_w
    ...
    for (int n_idx = 0; n_idx < N; n_idx++) {
        for (int k_idx = 0; k_idx < K_flat; k_idx++) {
            ...
            // 1. Calculate Input Channel (c)
            int c = k_idx / k_flat_size;
            // 2. Calculate Spatial Position
            ...
            // Stores R, G, B weights sequentially in the same column (n_idx)
    ...
    int M_limit_in_tile) 
{
    ...
    for (int m = 0; m < TM; m++) {
        ...
        // Loop calculates which channels fit in the current 32-element vector window
        int start_channel = k_start / spatial_kernel_size;
        int end_channel = (k_start + TK + K_w) / spatial_kernel_size;

        // Iterate through all valid channels (e.g., c=0 (Red), c=1 (Green), c=2 (Blue))
            ...
            for (int kh = 0; kh < K_h; kh++) {
                ...
                if (placement_shift >= 0) {
                    if (placement_shift < TK) {
                        ...
                        sim_vector_add(v_accum, v_row, TK); //stacking
                    }
                } else {
...
// Load Partial Sums
static void load_c_tile_partial(float* sc_C, TensorNCHW* output, int O_h, int O_w, int i_start, int j_start, int TM, int TN) {
    for (int r=0; r<TM; r++) for (int c=0; c<TN; c++) {
        int p_idx = i_start + r; int ch = j_start + c;
        if (p_idx < O_h * O_w && ch < output->c) {
            sc_C[r*TN + c] = output->data[ch*output->h*output->w + p_idx];
        } else {
            sc_C[r*TN + c] = 0.0f;
        }
    }
...
/*
================================================================================
 OUTPUT STATIONARY ORCHESTRATOR
================================================================================
*/
...
//     float sc_A[TM * TK]; // scA is a 32x32 matrix, it will be broken down into 1x32 vector registers in the hardware
//     float sc_B[TK * TN]; //kernel
//     float sc_C[TM * TN]; //output
...
//                 int active_N = (j + TN > N) ? (N - j) : TN; //calculate Active Output Channels in this tile
                
//                 // Process patches in batches of TM (32)
//                 for (int ii = 0; ii < patches_per_block; ii += TM) { // ii goes 0,32,64,96…
//                     int patches_remaining = patches_per_block - ii;
//                     int current_batch_size = (patches_remaining > TM) ? TM : patches_remaining;
//                     // Calculate Active Patches in this tile (for Stats)
//                     // assuming batch size is representative of SA row activation
//                     int active_M = current_batch_size;

//                     memset(sc_C, 0, TM * TN * sizeof(float));

//                     for (int k = 0; k < K; k += TK) {
//                         load_b_tile(sc_B, B_matrix, K, N, k, j, TK, TN); // kernels
                        
//                         // Fill sc_A row-by-row for valid patches
//                         for (int m = 0; m < current_batch_size; m++) { 
//                             // Fill the buffer sc_A (which is 32 rows×32 cols) to prepare it for the Systolic Array. 
//                             // Row m: Corresponds to one output patch.
...
//                             // Call vector transform for THIS specific patch 'm' (single output patch)
...
//                         // --- UTILIZATION STATS CALCULATION ---
//                         // 1. Capacity: 32 rows * 32 cols
//                         g_stats.total_pes += (TM * TN);
//                         g_stats.total_cycles++;

//                         // 2. Active Data:
//                         // Rows: active_M (number of valid patches in this batch)
//                         // Cols: active_N (number of valid kernels)
//                         // Depth: How much of TK is valid?
//                         //   If K=27, TK=32 -> active_K = 27.
//                         //   If K=50, Pass 1: active_K=32. Pass 2: active_K=18.
//                         int k_remaining = K - k;
//                         int active_K = (k_remaining > TK) ? TK : k_remaining;
                        
//                        //Active Weights Loaded / Total Weights Slot
                        
//                         long ops_in_this_tile = (long)active_M * active_N * active_K;
//                         long capacity_in_this_tile = (long)TM * TN * TK;
                        
//                         // Simplest: How many PEs are doing useful work?
//                         // If K maps to rows, then Active Rows = active_K.
//                         // Active Cols = active_N.
//                         // Active Patches (Time) = active_M.
                        
//                         // Just track average PE fill rate.
//                         // Assuming K is the spatial dimension of SA (Rows).
//                         g_stats.active_pes += (long)active_K * active_N; // Used PEs in this cycle
//                         log_gemm_step(active_M, active_N, active_K);
//                     }
...
/*
================================================================================
 WEIGHT STATIONARY ORCHESTRATOR
================================================================================
*/
void conv2d_nchw(TensorNCHW* output, TensorNCHW* input, KernelTensor* kernel, ConvParams* params) {
    init_analysis(); 
    ...
    // Note: We do NOT memset output to 0 here because we rely on the k loop initialization logic below.
    // However, for safety in C simulation, clear it once.
    memset(output->data, 0, output->n * output->c * O_h * O_w * sizeof(float));

    int N = kernel->cout; int K = kernel->cin * kernel->kh * kernel->kw; 
    int TM = SA_TM, TN = SA_TN, TK = SA_TK;
    int T_h = SCPAD_DIM; int T_w = SCPAD_DIM;

    int K_eff_h = (kernel->kh - 1) * params->dilation + 1;
    int BLK_H = (T_h - K_eff_h) / params->stride + 1;
    int BLK_W = (T_w - ((kernel->kw - 1) * params->dilation + 1)) / params->stride + 1;
    if (BLK_H < 1) BLK_H = 1; if (BLK_W < 1) BLK_W = 1;

    float* B_matrix = flatten_kernels_to_B_matrix(kernel);
    float* sc_RAW = (float*)malloc(kernel->cin * T_h * T_w * sizeof(float));
    if (!sc_RAW) { free(B_matrix); return; }

    float sc_A[TM * TK]; float sc_B[TK * TN]; float sc_C[TM * TN];
    int patches_per_block = BLK_H * BLK_W;

    for (int h_out = 0; h_out < O_h; h_out += BLK_H) {
        for (int w_out = 0; w_out < O_w; w_out += BLK_W) {
            
            // 1. Load Spatial Input Tile (Covers all Batches for this block)
            int in_h = h_out * params->stride - params->padding;
            int in_w = w_out * params->stride - params->padding;
            load_spatial_tile(sc_RAW, input, kernel->cin, T_h, T_w, 0, in_h, in_w);

            for (int j = 0; j < N; j += TN) {
                int active_N = (j + TN > N) ? (N - j) : TN;

                // --- WEIGHT STATIONARY LOOP REORDERING ---
                // Outer Loop: K (Depth/Weights). Load Weights ONCE.
                for (int k = 0; k < K; k += TK) {
                    
                    // 2. Load Weight Tile (Stationary for all batches)
                    load_b_tile(sc_B, B_matrix, K, N, k, j, TK, TN);
                    
                    int k_rem = K - k;
                    int active_K = (k_rem > TK) ? TK : k_rem;

                    // Inner Loop: Batches (Patches)
                    // Stream all patches against the currently loaded weights.
                    for (int ii = 0; ii < patches_per_block; ii += TM) {
                        ...
                        if (k == 0) {
                            // First pass: Reset local accumulator
                            memset(sc_C, 0, TM * TN * sizeof(float));
                        } else {
                            // Subsequent passes: Load Partial Sums from Main Memory
                            // This calculates the global patch indices for this batch
                            int start_patch_global = (h_out + (ii/BLK_W)) * O_w + (w_out + (ii%BLK_W)); // Approx
                            // Accurate global load logic required inside the function
                            // Using simplified approach: iterate m to find global index
                            
                            // NOTE: For efficient C simulation we do it row by row below or complex logic.
                            // Here we just zero sc_C because we will perform Read-Modify-Write in the STORE phase 
                            // or we need to load it here.
                            // Correct Weight Stationary requires loading sc_C here.
                            
                            // For simulation simplicity: We will clear sc_C, run GEMM, then ADD to memory in store phase.
                            memset(sc_C, 0, TM * TN * sizeof(float));
                        }

                        // 4. Construct Input Matrix A (for this batch)
                        for (int m = 0; m < current_batch_size; m++) {
                            int local_patch_idx = ii + m; 
                            int r_local = local_patch_idx / BLK_W;
                            int c_local = local_patch_idx % BLK_W;
                        ...
                        // 6. Update Output Memory (Read-Modify-Write for Partial Sums)
                        for (int m = 0; m < current_batch_size; m++) {
                             int local_patch_idx = ii + m; 
                             int r_local = local_patch_idx / BLK_W;
                             int c_local = local_patch_idx % BLK_W;
                             ...
                             for (int c = 0; c < TN; c++) {
                                 int ch = j + c;
                                 if (ch < output->c) {
                                     int out_addr = ch * output->h * output->w + p_idx;
                                     // ACCUMULATE RESULT: Mem = Mem + New_Partial
                                     // (If k=0, Mem was 0'd at start. If k>0, Mem holds prev sum)
                                     output->data[out_addr] += sc_C[m * TN + c];
                                 }
                             }
                        }
                    } // End Batch Loop
