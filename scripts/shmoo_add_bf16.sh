...
update_md() {
  ...
  if [[ ! -f "$MD_FILE" ]] || ! grep -q '^bf16 adder area & power report' "$MD_FILE"; then
    cat >"$MD_FILE" <<'HDR'
bf16 adder area & power report for different clock speed (MHz)
