...
def parse_file(filepath):
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            elif "Average ULP error:" in line: res['avg_ulp'] = float(line.split(":")[1].strip())
            elif "Number of cases with Integer ULP > 1:" in line: res['ulp_gt_1'] = int(line.split(":")[1].strip())
...
def plot_combined_architectures(t_x, t_max, t_gt1, t_pass, t_avg, f_x, f_max, f_gt1, f_pass, f_avg, title, out_name):
    ...
    f_x_mapped = [x for x in f_x]
    all_x = list(set(t_x + f_x_mapped))
    
    # 1. Max ULP
    axs[0,0].plot(t_x, t_max, marker='o', color='red', label='4-Input CC')
    axs[0,0].plot(f_x_mapped, f_max, marker='x', color='blue', label='4-Input No CC')
    ...
    # 2. Avg ULP
    axs[0,1].plot(t_x, t_avg, marker='o', color='red', label='4-Input CC')
    axs[0,1].plot(f_x_mapped, f_avg, marker='x', color='blue', label='4-Input No CC')
    ...
    # 3. ULP > 1
    axs[1,0].plot(t_x, t_gt1, marker='o', color='red', label='4-Input CC')
    axs[1,0].plot(f_x_mapped, f_gt1, marker='x', color='blue', label='4-Input No CC')
    ...
    # 4. Pass Rate
    axs[1,1].plot(t_x, t_pass, marker='o', color='red', label='4-Input CC')
    axs[1,1].plot(f_x_mapped, f_pass, marker='x', color='blue', label='4-Input No CC')
...
for (arch, mode), dirs in groups.items():
    ...
    if arch == 'comparison':
        t_dirs = [os.path.join(d, "adder_cc") for d in dirs]
        f_dirs = [os.path.join(d, "adder_no_cc") for d in dirs]
        
        # Aggregate Data
        t_x, t_max, t_gt1, t_pass, t_avg = aggregate_data(t_dirs, "output_precision_{}.txt", range(0,8))
        ...
        plot_combined_architectures(
            ...
            f"Comparison ({mode}) - CC vs No CC Architectures", 
            f"graph_comparison_combined_{mode}.png"
        )
        
        # Export Separated Graphs
        plot_single_architecture(t_x, t_max, t_gt1, t_pass, t_avg, f"Comparison ({mode}) - CC Adder Only", "Mantissa Size", f"graph_comparison_cc_separated_{mode}.png", 'red')
        plot_single_architecture(f_x, f_max, f_gt1, f_pass, f_avg, f"Comparison ({mode}) - No CC Adder Only", "Precision Bits", f"graph_comparison_no_cc_separated_{mode}.png", 'blue')
